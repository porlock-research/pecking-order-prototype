...
*   **Consequences:**
    ...
    *   **Follow-up (out of scope here):** include the recipient's current JWT in *all* push payloads in `apps/game-server/src/push-triggers.ts`, not just `sendGameEntryPush`. The SW already has the `if (data.token && data.url) { caches.open('po-tokens-v1')... }` branch (`apps/client/src/sw.ts:74-86`); piggybacking on existing notifications keeps Cache API self-refreshing without any new user-visible pushes. JWTs are ~400 bytes and web-push payloads are encrypted, so this is safe per channel.
    *   **Files:** `apps/client/src/App.tsx` — `updatePwaManifest` signature + new call from `applyToken`.
