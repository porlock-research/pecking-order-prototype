...
## [ADR-143] L3-Pregame — Sibling Actor for Pregame Engagement

*   **Date:** 2026-04-21
*   **Status:** Accepted
*   **Context:** L2's `preGame` state was an atomic state with no event handlers — every `SOCIAL.*` / `VOTE.*` / `GAME.*` event from a connected client was silently dropped (l2-orchestrator.ts:125-149 prior to this change). The lobby waiting room is the only pregame surface; once a player connects via the magic link, they hit a Pulse client with an empty chat, disabled input, and no interactive surface. Eager players who arrived early had no way to engage with the cast or other early arrivers. Two prior ADRs explicitly deferred this:
    *   **ADR-103 (Playtest Polish — Pregame UX):** "Pregame UX" section landed cosmetic chrome only — progress bar hidden, GM welcome messages, ticker countdown — no interactive surfaces.
    *   **ADR-105 (Bio Q&A System):** "Progressive reveal, gamification, and 'confessional booth' UX deferred to future iterations."

*   **Decision:** Add an L3-level sibling actor `l3-pregame` invoked by L2 in `preGame`, following ADR-026's invoke-with-`setup({actors})`-registration pattern (single machine, no polymorphic dispatch, so `invoke` not `spawn`). Lifetime is bound to the invoking state — born on `preGame` entry, stops automatically on transition to `dayLoop` (via `SYSTEM.WAKEUP` or `ADMIN.NEXT_STAGE`). Zero parent-awareness in the child; pure event-driven reactor.

    **Pregame data is journal-only and does not carry into game state.** This is the load-bearing call. Pregame interactions emit `FACT.RECORD` upward (PREGAME_PLAYER_JOINED, PREGAME_REVEAL_ANSWER) → L2's new preGame `FACT.RECORD` handler → `persistFactToD1` → `GameJournal` with `day_index = 0`. A parallel `pregame: { revealedAnswers, players }` slice lives in the SYNC payload only when `phase === 'pregame'`; the moment L2 transitions to `dayLoop`, the invoked actor stops, the slice disappears, and Day 1 chat is fresh. The journal in D1 is the only persistent record.

    **Deliberate departure from the standard fact→ticker→narrator pipeline** (see memory `reference_facts_ticker_pipeline.md`). PREGAME types are added to `JOURNALABLE_TYPES` (which gates `factToTicker` + `handleFactPush` per `finite-journalable-gates-ticker-and-push`), but `factToTicker` and `handleFactPush` have no handlers for the new types — both fall through to safe no-op defaults. Tests in `apps/game-server/src/__tests__/journalable-facts.test.ts` codify this intentional silence so a future default handler can't accidentally start emitting tickers/pushes for pregame facts.

*   **Consequences:**
    *   **Additive change.** L2's `preGame` state gains `invoke`, a `FACT.RECORD` handler mirroring `activeSession`'s, an additional `sendTo('l3-pregame', ...)` action on `SYSTEM.PLAYER_JOINED` after the existing roster mutation, and a `PREGAME.*` wildcard guard. No existing event paths changed. `Events.Pregame.PREFIX` added to L1's prefix allowlist (`ws-handlers.ts`); without it, REVEAL_ANSWER would be dropped at L1 (the trap that previously caught Spec C — see `finite-events-prefix-l1-allowlist`).
    *   **DO snapshot round-trip verified.** XState v5 invoke + `setup({ actors: { pregameMachine } })` registration round-trips cleanly. Reveals + players context survive serialization through DO hibernation. WAKEUP cleanly stops the child. Upgrade-time scenario (snapshot from old code that lacks `children['l3-pregame']`) handled gracefully — no validator-clear-snapshot pattern needed because pregame engagement degrades silently rather than crashing the L2 machine. Verified by 5 tests in `l3-pregame-snapshot.test.ts`.
    *   **Three Pulse surfaces shipped on top of this:** (1) **Cast Dossier** — pregame chip-tap opens a sheet showing persona portrait + bio + Q&A answers (ADR-105 data), reading entirely from the existing roster. (2) **First Impressions** — self-only "reveal one of your own QA answers" action; renders as a `NarratorLine`-style entry in the otherwise-empty pregame chat. One-shot per player, server-enforced. (3) **Joined-the-cast lines** — bespoke gold-ringed display-typed arrival lines (per `/impeccable:bolder` treatment) as players connect.
...
## [ADR-145] Validate manifests at /init + handle spawned-actor errors defensively
*   **Date:** 2026-04-21
*   **Status:** Accepted
*   **Context:** A bug that silently created zombie DOs on staging — traced via `INSPECT.SUBSCRIBE` — exposed three soft spots in our manifest → orchestrator pipeline. (1) The `/init` HTTP handler raw-forwarded the JSON body to the L2 actor without running `GameManifestSchema.safeParse()`, so malformed/incomplete manifests were accepted silently. (2) `DynamicManifestSchema.schedulePreset` was declared as required (`SchedulePresetSchema`) with no default — but only the lobby form actually enforced the field. Any ad-hoc script or direct curl that omitted it got through. (3) The spawned `game-master` actor crashed in `generateDayTimeline(undefined, …)` on such manifests; L2 had no `onError` for the spawn, so the `xstate.error.actor.game-master` event arrived at L2 but wasn't consumed by any transition. L2 then entered a degraded state: subsequent events still *arrived* at the machine (verified in INSPECT), but state transitions (including `activeSession.waitingForChild → running` via `INTERNAL.READY`, and admin injects like `START_CONFESSION_CHAT`) silently no-op'd. From the outside, SYNC kept working (L3 was happy), so the wedge was invisible. The earlier guardrail `finite-confession-admin-inject-no-op` had misdiagnosed this as a `sendTo` child-registry resolution failure; INSPECT showed events were routing fine — L2 was frozen.
*   **Decision:**
    1.  **Validate at the HTTP boundary.** `handleInit` (`apps/game-server/src/http-handlers.ts`) now runs `GameManifestSchema.safeParse(json.manifest)` and returns `400 INVALID_MANIFEST` with `issues: ZodIssue[]` on failure. The *parsed* manifest is then forwarded to the actor, so any schema defaults (below) are applied before `SYSTEM.INIT`.
    2.  **Ergonomic default on `schedulePreset`.** `DynamicManifestSchema.schedulePreset: SchedulePresetSchema.default('DEFAULT')` (`packages/shared-types/src/index.ts`). Scripts that forget the field now get `'DEFAULT'` filled in automatically instead of bricking the GM. The schema-as-documentation still says "you should set this," but absence is no longer load-bearing.
    3.  **Spawned-actor error handler at L2.** Top-level `on: { 'xstate.error.actor.game-master': { actions: log(...) } }` on the `pecking-order-l2` machine. Consuming the event keeps L2 live (the transition no-op problem was caused by the error being uncaught), and the action logs the error payload to Axiom so the crash is visible post-hoc instead of producing an invisible zombie DO.
*   **Consequences:**
    *   The entire class of "game created successfully but nothing works" bugs that depended on bad manifests is now unreachable via the documented creation paths. The symptom-level guardrail (`finite-confession-admin-inject-no-op`) can be retired once this ADR is a few weeks old with no recurrences.
    *   Establishes a template for other HTTP boundaries in the game server: **Zod-parse-then-forward** rather than raw-forward-then-hope. When adding new `/admin`, `/player-joined`, etc. handlers, the pattern is `safeParse → 400 on fail → use parsed data`. The reverse (accept raw, rely on actor guards) is what this ADR supersedes for `/init`.
    *   Establishes a template for spawned actors in L1/L2: **every `spawn()` site should have a corresponding `xstate.error.actor.<id>` handler on the parent** — minimum: log the error to Axiom. XState v5's default error propagation leaves the parent in a degraded state that's hard to distinguish from a live machine (INSPECT confirms events arrive; the machine just doesn't act on them). Audit remaining spawn sites (`spawnGameMasterIfDynamic`, `spawnVotingCartridge`, `spawnGameCartridge`, `spawnPromptCartridge`, `spawnDilemmaCartridge`) when touching them — add the handler opportunistically rather than as a blocking backfill.
    *   Shared-types `.default('DEFAULT')` on `schedulePreset` means the **schema no longer documents the field as strictly required**. Future readers should check the lobby UI (`apps/lobby/app/components/DynamicRulesetBuilder.tsx:71` defaults it explicitly) to see the intended user-facing constraint.
    *   Validation adds a few ms per `/init`. Negligible (the POST is not latency-sensitive) but noted.
    *   **Files:** `apps/game-server/src/http-handlers.ts`, `apps/game-server/src/machines/l2-orchestrator.ts`, `packages/shared-types/src/index.ts`, `.claude/guardrails/finite-confession-admin-inject-no-op.rule` (retained with updated diagnosis; can be deleted once no incidents recur for ~30 days).

## [ADR-146] Server-side WS reject rate-limit + close code 4008 (extends ADR-088/089)
*   **Date:** 2026-04-23
*   **Status:** Accepted (extends ADR-088, ADR-089)
*   **Context:** ADR-088 added the client-side `useGameEngine.onClose` redirect for codes 4001/4003 and ADR-089 added the server-authoritative `fetchActiveGames` purge. Both were client-side mitigations against PWA → archived-game reconnect storms. The 2026-04-21 PO playtest surfaced a residual case: 30+ WS reject events for the same playerId over 3 minutes, even with the redirect handler in place. Two failure modes: (1) backgrounded PWA whose `window.location.pathname` no longer matched `/game/CODE`, so the redirect short-circuit didn't fire and partysocket auto-reconnected indefinitely; (2) suspended PWA tabs that never received the redirect at all. Either way: client-only mitigations are insufficient when the client is in a degraded state.
*   **Decision:** Add a server-side defense-in-depth backstop. `apps/game-server/src/ws-handlers.ts` keeps a module-scoped `REJECT_CACHE: Map<key, {count, firstAt}>` per DO. `recordReject(key)` is called on both reject branches (invalid token at line 95, invalid playerId at line 104) and returns `'first' | 'repeated' | 'permanent'`. Key is `t:<token-suffix-16>` for token rejects, `p:<playerId>` otherwise. After 4 rejects in 30s, the close code escalates from 4001/4003 to **4008** ("Repeated invalid X — stop reconnecting"). Client `useGameEngine.onClose` treats 4008 identically to 4001/4003 (clear tokens, redirect to `/`) plus appends `&reason=rejected` to the URL.
*   **Consequences:**
    *   Runaway reconnect storms self-cap at 4 attempts per token per DO per 30s window. Production DOs stay warm long enough for the cache to be effective; partysocket's auto-reconnect sees the permanent close and respects it (does not reconnect on 4008).
    *   `REJECT_CACHE` is in-memory per-DO, not durable. A DO recycle wipes the counter — acceptable softening, since the goal is protection against a runaway client, not DDoS-grade enforcement.
    *   Local `wrangler dev` recycles the workerd isolate aggressively, so unit-style "send 8 invalid connections in a row" tests show partial cycles (4003-4003-4003-4008 then reset). Production behavior is enforced by the unit test (`apps/game-server/src/__tests__/ws-reject-rate-limit.test.ts`) which exercises `recordReject` deterministically with `vi.useFakeTimers()`.
    *   **Future audit:** when adding a new reject path on a WS connection, route it through `recordReject` so it gets the same escalation. Don't ship raw `ws.close(40xx)` calls without the cache check.
    *   **Files:** `apps/game-server/src/ws-handlers.ts`, `apps/client/src/hooks/useGameEngine.ts`, `apps/game-server/src/__tests__/ws-reject-rate-limit.test.ts`.

## [ADR-147] Lobby HTTP endpoints return 200 + null/empty body for benign no-data states
*   **Date:** 2026-04-23
*   **Status:** Accepted
*   **Context:** `/api/refresh-token/[code]` and `/api/my-active-game` returned 401/400/403/404 for several states that are not actual server errors — "no session cookie," "game not started yet," "user has no invite for this game," "anon user opened the PWA." Sentry's browser SDK auto-instruments fetch responses ≥400 as captured errors, which drowned the staging dashboard with `PECKING-ORDER-STAGING-2` (refresh-token 400, 14 events, 3 users) and `PECKING-ORDER-STAGING-18` (my-active-game 401, every anon home-page open). None were bugs. The client's recovery flow already treated these as no-op signals and fell through to the lobby `/play/` redirect — the 4xx contract only existed because it was the easy default.
*   **Decision:** Lobby HTTP endpoints distinguish between **(a)** genuine errors (404 for invalid invite codes, 500 for actual server problems, 403 for cross-account access) and **(b)** benign "no data available right now" states (no session, game not started, user not in game). For (b), return `200` with a null/empty body and a `reason` field describing why. The client checks `reason` and treats the response identically to a 4xx; behavior is unchanged but Sentry stays signal-rich. `reason` lets the client distinguish "we asked the server and the user has no active games" from "the user has no session" — the latter must NOT trigger token purges (cached game tokens may still be valid).
*   **Examples:**
    *   `GET /api/refresh-token/<CODE>` for unauth/no-invite/game-not-started → `200 { token: null, reason: 'unauthenticated'|'no_invite'|'game_not_started' }`. 404 still returned for `game_not_found` (genuine "wrong code").
    *   `GET /api/my-active-game` for unauth → `200 { games: [], reason: 'unauthenticated' }`. Authed-with-zero-games → `200 { games: [] }` (no reason).
*   **Consequences:**
    *   Sentry dashboard signal-to-noise improves materially. Real 4xx still surface; benign no-data does not.
    *   Establishes a contract for new lobby endpoints: **default to 200 + null/empty for "user just doesn't have access right now."** Reserve 4xx for genuine errors. When in doubt, ask "is this a server bug or a user-state condition?" — if user-state, it's 200+null.
...
## [ADR-149] PWA cold launch — bake JWT into manifest `start_url` (reverses ADR-088 part 2)
*   **Date:** 2026-04-26
*   **Status:** Accepted (reverses ADR-088 decision #2)
*   **Context:** ADR-088 (2026-03-02) made the PWA's `start_url` static at `/` so cold launch always lands on the launcher and discovers games via `fetchActiveGames`. The cited driver was a stale-game failure: PWAs with a baked `/game/CODE?_t=<jwt>` start_url would loop on WS reject when the game later archived. The 2026-04-26 M4P9BG playtest surfaced what that decision cost: a player ("TMI Tammy" / `mack@merchantsofplay.com`) authed in Chrome iOS, installed the PWA via "Add to Home Screen," and on cold launch the iOS standalone Web App container had **no** recoverable identity. All four storage paths in `runAsyncRecovery` returned empty (Sentry tag `lobby-recover-none`), the cascade fell through to `${LOBBY_HOST}/j/CODE`, and `claimSeat`'s anon path silently minted a brand-new `User` row + duplicate Invite (Felix Stage at slot 4 alongside her real TMI Tammy at slot 3). The structural problem: iOS Web App standalone containers are isolated WebKit instances, so cookies / localStorage / Cache API are subject to install-time copy heuristics that fail when auth happened outside Mobile Safari. The only artefact that survives the install boundary unconditionally is the manifest's `start_url`.
*   **Decision:** Restore the pre-ADR-088 behaviour where `applyToken` rebuilds the data: URL manifest with `start_url: /game/CODE?_t=<jwt>`. iOS reads the manifest at "Add to Home Screen" time and bakes that URL in. Cold launch hits the existing fast path at `App.tsx` (`if (gameCode && urlToken) { applyToken(...) }`) and is in the game with zero dependency on cookies, localStorage, Cache API, or cross-origin lobby refresh.
    *   `updatePwaManifest()` now takes optional `(gameCode, jwt)`. With both, it bakes the tokenised start_url. Without (e.g. unauth boot at `/`), it falls back to `${origin}/` so a launcher-only install still works for users who have not yet entered a game.
    *   Re-call `updatePwaManifest(gameCode, jwt)` from inside `applyToken`, after the existing localStorage / Cache API / cookie writes. The most-recently-entered game is always reflected in the manifest.
*   **Why reversing ADR-088 is now safe:** The stale-game class of failure is closed at three independent layers shipped after ADR-088:
    *   **ADR-088 itself** — `useGameEngine.onClose` redirects to `/` on close codes 4001 / 4003. Stale start_url → DO rejects WS → client navigates to launcher.
    *   **ADR-089** (2026-03-03) — `fetchActiveGames` + `purgeInactiveTokens` ensures the client never even attempts a WS to a game the server says is inactive. Cold launch with a stale token shows the archived-game UI before any reconnect storm could form.
    *   **ADR-146** (2026-04-23) — server-side WS reject rate-limit with close code 4008. Even if the client is degraded and ignores 4001/4003, the server cuts off after 4 attempts in 30s.
*   **Consequences:**
    *   **New installs**: iOS cold launch loads `/game/CODE?_t=<jwt>` → applyToken fast path → in-game. No identity-loss path through `/j/CODE`.
    *   **Pre-existing installs from the ADR-088 era**: unchanged. Their start_url is still `/`; recovery cascade still applies. A reinstall picks up the new behaviour.
    *   **JWT expiry**: `dayCount × 2 + 7` days (per `apps/lobby/app/play/[code]/route.ts`). After expiry, the start_url's JWT fails the `exp` check at `App.tsx:443-446`, redirects to `/`, and existing recovery paths take over.
    *   **Multi-game players**: PWA opens to whichever game `applyToken` ran for most recently. Switching to a different active game is a manual navigation back to `/` (at which point `fetchActiveGames` lists active games). A future launcher chip / switcher can polish this; the regression vs ADR-088 is small (one re-tap) and the cold-launch reliability gain is large.
    *   **Stale game** is identical in shape to ADR-088, just one extra hop: cold launch → applyToken on a defunct game → fetchActiveGames says inactive → launcher with archived-game banner.
    *   **Bricked-icon edge case (verify on hardware):** if a baked-manifest cold launch hits a 4001/4003/4008 reject (e.g. `AUTH_SECRET` rotation, DO state loss, server-side ban), `useGameEngine.onClose` clears the per-game caches and redirects to `/?noRecover=1`. The client's in-memory `<link rel="manifest">` is updated to the launcher fallback at `/`, but iOS WebKit is widely reported to **freeze the manifest at PWA install time** — subsequent cold launches from the home-screen icon would still hit `/game/CODE?_t=<jwt>` and replay the reject → redirect cycle until the user reinstalls the PWA. JWT TTL (`dayCount × 2 + 7` days) bounds this; in the worst case the user sees a "RESTORING_SESSION..." flash before landing on the launcher every cold launch. ADR-088's `start_url: '/'` made this case impossible because the icon always pointed at the launcher; ADR-149 reintroduces it as an edge. Behaviour should be verified on a real iOS device during the playtest pass — if iOS *does* re-read the manifest on cold launch in the relevant version, this downgrades from "bricked icon until reinstall" to "cosmetic flash". Either way the JWT-in-pushes follow-up (see below) would shorten the recovery window further by keeping the in-PWA Cache API fresh.
    *   **Sentry observability:** the URL-token fast path is tagged `'manifest-cold-launch'` when `display-mode: standalone` matches at init, vs. `'transient'` for browser-tab arrivals via the lobby redirect. Both code paths are identical at runtime (same `applyToken(urlToken, gameCode, …)`), but the tag distinction is what lets us answer "is the ADR-149 fix actually firing in production?" without inferring from the absence of `lobby-recover-none` events.
    *   **JWT-in-URL leakage surfaces (acceptable):** the manifest-baked `start_url` puts the JWT in (a) one cold-launch entry in browser history (then immediately stripped via `replaceState` in `applyToken`), (b) Cloudflare Pages access logs for the cold-launch hit, (c) Sentry breadcrumbs if anything throws between cold-launch nav and the URL strip, (d) the share sheet if the user shares the page during that ~tick. Risks are bounded: token TTL is `dayCount × 2 + 7` days, and the JWT only authorises one player slot in one game (no cross-game authority, no privilege escalation). This is also the same shape as the existing lobby `/play/CODE → /game/CODE?_t=<jwt>` redirect that has been in production for months. Acceptable.
    *   **What this *does not* fix (and isn't trying to):** the SW Cache API self-refresh branch (`apps/client/src/sw.ts:74-86, `if (data.token && data.url) { caches.open('po-tokens-v1')... }`) is dormant for in-game pushes today — `apps/game-server/src/push-triggers.ts` (DM/MENTION/REPLY/NUDGE/WHISPER/SILVER) does not include `token` in any payload, only the one-shot `sendGameEntryPush` does. ADR-149 does not change that, and intentionally so: the manifest-baked `start_url` *replaces* the SW-Cache-API recovery as the structural cold-launch path, it does not supplement it. Consequently, a returning player whose JWT has expired (>21d unused for a 7-day game) and who has not received a fresh game-entry push lands on the same `runAsyncRecovery` cascade as before — the manifest gives them a fast path inside the JWT lifetime, not beyond it. A separate small change (include `token` in selected `push-triggers.ts` payloads) is worth doing on its own merits to keep Cache API fresh between game days, but it's *not* a prerequisite for ADR-149 and not a regression caused by ADR-149 either.
    *   **Files:** `apps/client/src/App.tsx` — `updatePwaManifest` signature + new (gameCode-guarded) call from `applyToken`. New unit test: `apps/client/src/__tests__/pwa-manifest.test.ts` (5 cases covering the both-args / no-args / one-arg / DOM-update-in-place / cold-launch-regex-coupling contracts).

## [ADR-150] Pregame NUDGE — open the "hey, I see you" ping before Day 1 (narrows ADR-143)
*   **Date:** 2026-05-02
*   **Status:** Accepted (narrows ADR-143's "silver/nudge/DMs stay closed" rule)
*   **Context:** ADR-143 introduced `l3-pregame` with a deliberate stance: pregame's MAIN channel carries `['CHAT', 'REACTIONS', 'WHISPER']` but **not** NUDGE/SILVER/DMs ("the design is 'meet the cast, gossip before Day 1 starts' without the economy yet"). The 2026-04-29 SBCSJT playtest exposed an engagement gap during long pregames (PLAYTEST_NEXT preset has 10h+ of pregame between create and Day 1): players had nothing to do beyond chat-and-whisper, with no per-player attention mechanic. WHISPER is intrigue-only (text required, sender-target pair, persisted to journal) which makes it heavyweight for a "are you there?" ping. The Pulse client UI is already capability-gated — `HintChips.tsx:29` (`/nudge` chip), `CommandPicker.tsx:20` (`/nudge` slash-command), and the tap-popover all read `MAIN.capabilities`. Adding NUDGE to pregame's channel cap would auto-light those surfaces.
*   **Decision:** Open NUDGE in pregame. Specifically:
    1.  `PREGAME_MAIN_CHANNEL.capabilities` becomes `['CHAT', 'REACTIONS', 'WHISPER', 'NUDGE']` in `apps/game-server/src/machines/l3-pregame.ts`.
    2.  Pregame gets a `canNudge` guard (alive sender + alive target + self ≠ target + cap present) — modeled on `l3-social`'s `isNudgeAllowed` but reading from pregame's `players` map rather than dayLoop's `roster`.
    3.  Pregame gets a `[Events.Social.NUDGE]` handler that emits a NUDGE fact via `sendParent` → L2 → journal+push pipeline. No state mutation in pregame context.
    4.  `l2-orchestrator.ts` pregame social allowlist forwards `Events.Social.NUDGE` to `l3-pregame` (was: WHISPER + SEND_MSG + REACT only).
*   **What stays closed in pregame:** SILVER and DMs. Silver requires the day-loop economy (silver budget per day, transfer caps); DMs require the channel-creation flow (slot tracking, invite mode). Neither has a sensible pre-Day-1 semantics. NUDGE is special because it has no state and no economy — pure attention signal that maps cleanly to "wake up, we're starting soon."
*   **Why no rate limit in pregame:** `l3-social.isNudgeAllowed` enforces per-(sender,target,day) once-per-day throttle via `nudgesThisDay`. In pregame there's no day boundary to anchor that throttle, and the design intent is "meet the cast" where repeat-pings are fine. If abuse emerges, add a session-scoped `nudgedInPregame: Record<string,true>` later — keep it out of v1.
*   **Consequences:**
    *   Engagement surface added without UI work — capability-gated client UI lights up automatically once SYNC carries the new caps array.
    *   ADR-143's "no economy in pregame" stance narrows: NUDGE is not economy (no resource transfer), it's pure attention. Future agents should read this ADR before re-opening SILVER or DMs in pregame; those have stronger reasons to stay closed.
    *   Push fact pipeline is unchanged — NUDGE fact already in `JOURNALABLE_TYPES`, `handleFactPush` already handles `FactTypes.NUDGE` with `intent: { kind: 'main' }` (per `project_push_triggers_v2`). Pregame nudges land as pushes in the same shape as day-loop nudges, with the same `kind: 'main'` click-routing limitation tracked in GH #134.
    *   **Files:** `apps/game-server/src/machines/l3-pregame.ts`, `apps/game-server/src/machines/l2-orchestrator.ts`. Tested via existing `l3-pregame`, `l3-channel-capabilities`, `event-coverage` suites (126 passing). No dedicated nudge test — followup if regressions surface.

## [ADR-151] Lobby-scoped tabloid theme via `[data-theme="reality-tv-tabloid"]`
*   **Date:** 2026-05-04
*   **Status:** Accepted
*   **Context:** The 2026-05-04 lobby redesign retired the purple/pink/gold ramp in favor of tabloid B+W + red. The natural-feeling implementation was a global token swap inside `:root, [data-theme="reality-tv"]` in `packages/ui-kit/src/theme.css` — but that block is the default theme for **every** consumer of `@pecking-order/ui-kit`. A global swap would have rebranded the client's legacy Vivid/Classic/Immersive shells (which use `--po-*` via `skin-*` Tailwind classes) by accident, even though the redesign was scoped to lobby. Pulse is isolated separately (`--pulse-*` tokens in `pulse-theme.css`, no `--po-*` consumption) so it isn't a concern, but the legacy shells are.
*   **Decision:** Add a parallel `[data-theme="reality-tv-tabloid"]` block to `theme.css` that **only overrides the color tokens that change** for the lobby variant — backgrounds, text ladder, accent (`--po-pink` becomes red), gold, borders, ink-tinted shadows, gradient. Typography, radii, motion, and status colors inherit from the default block. Apply via `<html lang="en" data-theme="reality-tv-tabloid">` in `apps/lobby/app/layout.tsx`. The default `:root, [data-theme="reality-tv"]` block (purple/pink/gold) is unchanged.
*   **Why this shape (and not the alternatives):**
    *   **Forking `theme.css` per-app** would duplicate every non-color token (radii, motion, fonts) and silently drift; one-source-of-truth is easier to maintain.
    *   **Hot-swapping at runtime via JS** (e.g., a `<ThemeProvider>` that injects a `<style>` tag) violates SSR for Next.js: the lobby's RSC tree renders without JavaScript and tokens must resolve at first paint.
    *   **App-scoped CSS imports** could shadow `--po-*` tokens but produce specificity conflicts with `@pecking-order/ui-kit/theme.css`. The data-theme attribute pattern leverages CSS's natural cascade — same selector specificity, last-wins by source order, predictable.
*   **Token names preserved across both blocks:** `--po-pink` carries red (`#d72638`) in the tabloid variant and hot pink (`#ec4899`) in the default. Same for `--po-gold` (slight retune `#fbbf24` → `#f7c52e`). This is deliberate: renaming would require sweeping every call site (`bg-skin-pink`, `text-skin-gold`, etc.) across both the lobby and the legacy shells, which would have made the redesign land much later. Token-rename to actual semantic names (e.g., `--po-red`) is a deferrable follow-up; both palettes use the same key names with different values.
*   **Consequences:**
    *   **Lobby pages** get the tabloid palette automatically via cascade. No per-page styling work for the chrome layer; per-page work is only needed for composition changes (masthead pattern, kicker eyebrows, rhythm strip — see `apps/lobby/.impeccable.md`).
    *   **Vivid / Classic / Immersive shells** continue inheriting the purple/pink/gold default. They remain visually consistent with their pre-redesign appearance.
    *   **Pulse** unchanged — separate token system, not affected by `--po-*` modifications.
    *   **`border-skin-rule` Tailwind utility added** in the same change — surfaces `--po-rule` (#2c2c2c) hairline divider, only defined in the tabloid block. Theme-agnostic in the preset; renders nothing under the default theme since `--po-rule` is absent there. New utility is safe for any theme to opt into later.
    *   **Future per-app theme variants** follow the same pattern: parallel `[data-theme="<app-tag>"]` block in `theme.css` + `<html data-theme="<app-tag>">` in the consuming app's root layout. Don't fork `theme.css`.
    *   **Email is the exception:** lobby's email templates use a paper-first mirror palette inlined as raw hex (Outlook normalisation strips CSS custom properties). Documented in `apps/lobby/.impeccable.md` "Color (email — paper-first)".
    *   **Files:** `packages/ui-kit/src/theme.css` (added tabloid block), `packages/ui-kit/src/tailwind-preset.js` (added `border-skin-rule`), `apps/lobby/app/layout.tsx` (data-theme attribute), `apps/lobby/.impeccable.md` (full brief). Commits: `97095a5` (brief), `93e7fa4` (token scope), `2b7661c` / `9257d6e` / `f808907` / `ca7937a` / `6f84ce2` (per-page surgery).
