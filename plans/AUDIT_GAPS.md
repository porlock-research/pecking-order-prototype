...
- **Spawned game/vote cartridge actors persist in L3 children map after cleanup** — `cleanupGameCartridge` and `cleanupCartridge` null the context ref but don't stop the spawned actor. The actor remains in `l3Snap.children['activeGameCartridge']` and L1's projection still finds it. This is currently acceptable (the client uses in-memory dismiss), but could cause stale data on reconnect if the game phase ended while the client was disconnected.
