...
export const finalsMachine = setup({
  ...
  actions: {
    recordVote: assign({
      votes: ({ context, event }) => {
        if (event.type !== VoteEvents.FINALS.CAST) return context.votes;
    ...
    emitVoteCastFact: sendParent(({ event }) => {
      if (event.type !== VoteEvents.FINALS.CAST)
  ...
  states: {
    active: {
      on: {
        [VoteEvents.FINALS.CAST]: { actions: ['recordVote', 'emitVoteCastFact'] },
