...
export const majorityMachine = setup({
  ...
  actions: {
    recordVote: assign({
      votes: ({ context, event }) => {
        if (event.type !== VoteEvents.MAJORITY.CAST) return context.votes;
    ...
    emitVoteCastFact: sendParent(({ event }) => {
      if (event.type !== VoteEvents.MAJORITY.CAST)
  ...
  states: {
    active: {
      on: {
        [VoteEvents.MAJORITY.CAST]: { actions: ['recordVote', 'emitVoteCastFact'] },
