...
export function factToTicker(fact: any, roster: Record<string, any>): TickerMessage | null {
  ...
  switch (fact.type) {
    ...
    // CONFESSION_PHASE_STARTED + CONFESSION_POSTED + CONFESSION_PHASE_ENDED
    // intentionally fall through to default null:
    // - PHASE_STARTED: surfaced via Pulse's ConfessionPhaseBanner (sticky strip
    //   below the header) and the booth's auto-open. A chat-stream narrator
    //   would be redundant and gets buried as soon as anyone sends a message;
    //   it also persists past phase close, misleading players who land cold.
